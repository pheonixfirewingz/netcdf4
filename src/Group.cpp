...
Group::Group(int id_) noexcept : id(id_) {
...
void Group::AddVariable(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    const size_t ndims = array->Get(isolate->GetCurrentContext(), 
        v8::String::NewFromUtf8(isolate, "length", v8::NewStringType::kNormal).ToLocalChecked())
        .ToLocalChecked()->Uint32Value(isolate->GetCurrentContext()).ToChecked();
    ...
    int new_id = -1;
    const int retval = nc_def_var(obj->id, *v8::String::Utf8Value(isolate, args[0]), 
                                   type, static_cast<int>(ndims), dimids.data(), &new_id);
...
void Group::GetName(v8::Local<v8::String> property, const v8::PropertyCallbackInfo<v8::Value>& info) {
    ...
    if (obj->get_name(name.data())) {
        info.GetReturnValue().Set(v8::String::NewFromUtf8(isolate, name.data(), v8::NewStringType::kNormal).ToLocalChecked());
    }
}
